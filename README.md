...
1. The input is processed through two sequential linear transformations and LeakyReLU activations: $$h_i=\operatorname{LeakyReLU}\left(W_i h_{i-1}+b_i\right),$$ for $i \in \{1,2\}$, where $h_0=x$, and $W_i$ and $b_i$ represent the weights and biases of the $i^{\text {th }}$ layer, respectively.
...
1. The latent input $z \in \mathbb{R}^n$ is transformed through a linear layer: $$h_1= \operatorname{LeakyReLU} \left(z W_1^T+b_1\right),$$ where $W_1$ and $b_1$ are the weights and biases of the first transformation layer, respectively.
2. The resulting hidden representation $h_1$ is further processed: $$h_2=\operatorname{LeakyReLU}\left(h_1 W_2^T+b_2\right),$$ utilizing another set of weights and biases $\left(W_2, b_2\right)$ for the second transformation.
